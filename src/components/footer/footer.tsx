  ...
  return (
    <>
      ...
      <section className="border-t border-black/20 relative w-full">
        ...
        {/* Contenedor del Mapa (no afecta CLS porque aparece tras interacción) */}
        {isMapOpen && (
          <div
            id="project-map"
            className="w-full mt-5 flex items-center justify-center bg-gray-200 rounded-lg shadow-lg overflow-hidden"
          >
            <iframe
      ...
      {/* ============ Footer principal (CLS FIX: altura reservada + contain) ============ */}
      <footer
        id="contact"
        className="bg-[#0D4C5F] flex items-center justify-center w-full min-h-[560px] md:min-h-[420px]"
        style={{ contain: "content" as any }} // aísla el layout interno (reduce layout shifts)
      >
        <section className="w-full max-w-[1400px] flex flex-col py-10 md:px-10 md:items-center md:justify-center">
          ...
          {/* CLS FIX: reserva altura para las tarjetas */}
          <div className="flex flex-col-reverse gap-4 mb-5 items-start md:w-full md:flex-row md:justify-between px-5 max-w-[1100px]">
            {/* Contractor Call-To-Action Card (componente propio) */}
            <div className="min-h-[300px] w-full md:max-w-md">
              <ContractorCard />
            </div>

            {/* Financing Card (altura estable) */}
            <div className="flex flex-col items-center justify-center gap-3 bg-gradient-to-t md:bg-gradient-to-r from-red-800 to-purple-800 text-center text-white p-6 rounded-lg shadow-lg w-full md:max-w-md min-h-[300px]">
          ...
          {/* Logo con dimensiones explícitas (evita reflow) */}
          <div className="w-full px-5">
            <img
              src="/assets/images/IdentidadSVG/LogoBlanco.svg"
              alt="New Gen Patio Logo"
              className="p-2 pl-0 w-[65px] h-[80px]"
              width={65}
              height={80}
              decoding="async"
              loading="lazy"
          ...
          {/* Copyright */}
          <div className="w-full bg-white/40 h-[1px] my-5" />
