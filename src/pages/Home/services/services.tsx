...
type ServiceCard = {
  ...
  link: string;
  subtitle?: string;
  // ⬅️ Nuevo: Añadimos la clave de traducción original para el JSON-LD y el mapeo
  key: string; 
};

/* ========================= Datos base (Mapeados para i18n) ========================= */
const RAW_CARDS_KEYS = [
  {
  ...
  },
  {
    ...
    subtitle: "Landscaping | Concrete | Artificial Turf", // Subtítulo también debe traducirse
  },
];

/* ========================= Prefetch helpers (Sin cambios) ========================= */
// ... (Toda la lógica de prefetch helpers se mantiene sin cambios)
const canPrefetch = () => {
    // ... (código canPrefetch)
...
const Services: React.FC<ServicesProps> = ({ showQuoteButton = true }) => {
    // ⬅️ Usamos el namespace 'services' para los textos de la sección
    const { t } = useTranslation(['services', 'common']);

    const baseUrl = import.meta.env.BASE_URL || "/";
    const sectionRef = useRef<HTMLElement | null>(null);

    // Normaliza rutas y TRADUCE los textos de las tarjetas
    const serviceCards = useMemo<ServiceCard[]>(
        () =>
            RAW_CARDS_KEYS.map((c) => ({
                ...c,
                // ⬅️ Traducción de TÍTULO y SUBTÍTULO
                title: t(`cards.${c.key}.title`),
                subtitle: c.subtitle ? t(`cards.${c.key}.subtitle`) : undefined,
    ...
    const jsonLd = useMemo(() => {
        const itemListElement = serviceCards.map((c, i) => ({
            ...
            item: {
                "@type": "Service",
                // ⬅️ CRÍTICO: Usamos el título traducido, no el RAW_CARDS
                name: c.title, 
                image: origin ? `${origin}/${c.imageUrl.replace(/^\//, "")}` : c.imageUrl,
                url: `${origin}${c.link}`,
                // ⬅️ CRÍTICO: Usamos el subtítulo traducido
        ...
        return {
            ...
            name: t('json-ld-list-name', { defaultValue: "New Gen Patio - Services" }), 
        };
    }, [serviceCards, origin, t]);
    ...
    return (
        ...
        >
            <header className="text-center max-w-2xl">
                {/* ⬅️ Traducción: Título 1 (OUR SERVICES) */}
                <h2 id="services-heading" className="text-2xl font-semibold text-[#0d4754]">
                    {t('header-title-1')} 
                </h2>
                {/* ⬅️ Traducción: Título 2 (What We Offer) */}
                <p className="text-4xl font-semibold">{t('header-title-2')}</p>
            ...
            {showQuoteButton && (
                <FreeQuoteButton
                    questionText={t('common:quote-button-question', { defaultValue: "Got a project in mind?" })}
                    buttonText={t('common:quote-button-cta', { defaultValue: "Let’s Talk" })}
